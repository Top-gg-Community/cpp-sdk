...
class ratelimited : public std::runtime_error {
  ...
  inline ratelimited(const uint16_t retry_after)
      : m_retry_after(retry_after),
        std::runtime_error(
            "This client is ratelimited from further requests. Please try "
            "again later.") {}
...
class internal_client_error : public std::runtime_error {
  ...
  inline internal_client_error(dpp::http_error http_error, const char* message)
      : m_http_error(http_error), std::runtime_error(message) {}
...
class result {
  ...
  const std::function<T(nlohmann::json& json)> m_parse_fn;

  inline constexpr result(dpp::http_request_completion_t response,
                          std::function<T(const nlohmann::json& json)> parse_fn)
      : m_response(response), m_parse_fn(parse_fn) {}
