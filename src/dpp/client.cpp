...
dpp_client::dpp_client(dpp::cluster* cluster, const std::string& token): m_cluster(cluster) {
...
template<typename T>
void dpp_client::basic_request(const std::string& url, std::function<void(const result<T>&)> callback, std::function<T(const nlohmann::json&)> conversion_fn) {
...
void dpp_client::get_bot(const dpp::snowflake& bot_id, topgg::get_bot_completion_t callback) {
  basic_request<topgg::bot>("/bots/" + std::to_string(bot_id), callback, [](const auto& j) {
    return topgg::bot{j};
  });
}

void dpp_client::get_user(const dpp::snowflake& user_id, topgg::get_user_completion_t callback) {
...
void dpp_client::has_voted(const dpp::snowflake& user_id, topgg::has_voted_completion_t callback) {
