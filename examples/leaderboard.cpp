...
int main() {
  ...
  const auto [httpCode, responseBody] =
      api.get("/bots/" + api.getBotID() + "/votes");
  ...
  std::vector<std::pair<std::string, int>> usersByVotes(voteCounts.begin(),
                                                        voteCounts.end());
  std::sort(usersByVotes.begin(), usersByVotes.end(),
            [](const auto& a, const auto& b) { return a.second > b.second; });

  // Print the leaderboard
  std::cout << "Leaderboard:\n";
  for (size_t i = 0; i < usersByVotes.size() && i < 10; ++i) {
    std::cout << i + 1 << ". User " << usersByVotes[i].first << ": "
              << usersByVotes[i].second << " votes\n";
  }
