...
class User {
 public:
  User(const std::string& id, const std::string& username,
       const std::string& discriminator, const std::string& avatarUrl);
...
class BotStats {
 public:
  BotStats(int serverCount, const std::vector<int>& shardCount,
           const std::vector<int>& shardIds);
...
class UserVote {
 public:
  UserVote(const std::string& userId,
